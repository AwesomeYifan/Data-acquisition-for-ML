...
class Appr(object):
    ...
    def train(self,t,xtrain,ytrain,xvalid,yvalid, is_first_itr=False):

        # Update the next learning rate for each parameter based on their uncertainty
        params_dict = self.update_lr(is_first_itr)
    ...
    def update_lr(self,is_first_itr, lr=None, adaptive_lr=False):
        params_dict = []
        if is_first_itr:
