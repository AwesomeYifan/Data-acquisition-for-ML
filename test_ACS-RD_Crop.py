...
for _ in range(10):
    seller.reset()
    model = CModel()
    buyer = RD_Buyer(total_budget, n_classes, features, trainX, trainY, init_image_ids, model, seller)
...
# evaluation step
write_file = open(folder + "results/RD-results.csv", 'w')
for budget in budget_list:
    model = CModel()
    f = open(folder + "RD-" + str(budget), 'r')
    lines = f.readlines()
    for line in lines:
