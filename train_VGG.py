...
class Cifar10Dataset(Dataset):
    ...
    def __init__(self, root, train=True,
        ...
        # print("re-implementation of CIFAR-DATASET")
        if self.train:
        ...
        if self.train:
            ...
            init_image_ids = np.loadtxt(args.path_to_folder + "init.csv")
            init_image_ids = [int(v) for v in init_image_ids]
            more_image_ids = np.loadtxt(args.path_to_folder + args.file_name)
            more_image_ids = [int(v) for v in more_image_ids]
            selected_image_ids = init_image_ids.copy()
            selected_image_ids.extend(more_image_ids)
            selected_image_ids = list(dict.fromkeys(selected_image_ids))

            self.data = self.data[selected_image_ids]
            # print(self.targets)
            self.targets = np.asarray(self.targets)
            self.targets = self.targets[selected_image_ids]
...
if args.dataset == 'MNIST':
    ...
    # dataset_train = Cifar10Dataset('../data/CIFAR10', train=True, transform=train_transform)
    dataset_train = Cifar10Dataset('drive/My Drive/marketplace/data/CIFAR10', train=True, transform=train_transform)
    # print("drive file")
    train_loader = torch.utils.data.DataLoader(
        dataset_train,
    ...
    #             transforms.Normalize((0.424, 0.415, 0.384), (0.283, 0.278, 0.284))
    #         ]))
    dataset_test = Cifar10Dataset('drive/My Drive/marketplace/data/CIFAR10', train=False,
            transform=transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.424, 0.415, 0.384), (0.283, 0.278, 0.284))
            ]))
    ...
    if args.cutout:
        train_transform.transforms.append(Cutout(n_holes=args.n_holes, length=args.length))
    # dataset_train = datasets.CIFAR100('drive/My Drive/marketplace/data/CIFAR100', train=True, download=True, transform=train_transform)
    dataset_train = Cifar100Dataset('drive/My Drive/marketplace/data/CIFAR100', train=True, transform=train_transform)
    train_loader = torch.utils.data.DataLoader(
        dataset_train,
        sampler = None if args.classes_per_batch == 0 else NClassRandomSampler(dataset_train.train_labels, args.classes_per_batch, args.batch_size),
        batch_size=args.batch_size, shuffle=args.classes_per_batch == 0, **kwargs)
    # dataset_test = datasets.CIFAR100('drive/My Drive/marketplace/data/CIFAR100', train=True, download=True,
    #         transform=transforms.Compose([
    #             transforms.ToTensor(),
    #             transforms.Normalize((0.438, 0.418, 0.377), (0.300, 0.287, 0.294))
    #         ]))
    dataset_test = Cifar100Dataset('drive/My Drive/marketplace/data/CIFAR100', train=False,
            transform=transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.438, 0.418, 0.377), (0.300, 0.287, 0.294))
            ]))
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    print(test_error,end=',', flush=True)
    if epoch == args.epochs:
        acc = (100 - test_error) / 100
        print()
        print("final accuracy: " + str(acc))
        write_file = open(args.path_to_folder + "results/" + str(args.file_name) + '-results', 'w')
        write_file.write(str(acc) + "\n")
        write_file.flush()
        write_file.close()
